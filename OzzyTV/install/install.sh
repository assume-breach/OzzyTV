...
for a in "$@"; do
  case "$a" in
    ...
    *) die "unknown option '$a' (try --no-kiosk or --uninstall)" ;;
  esac
done
...
Ozzy TV is installed.   [$(cat "$APP_DIR/BUILD")]
  ...
  Optional — put $MEDIA_DEFAULT on the network, so programmes arrive over wifi
  instead of on a memory stick:

        sudo ./install/share.sh

     Only that one folder is shared, to one named user, with a password of its
     own. The television picks up new files within a few seconds.
