...
def install(tmp_path, venv):
    ...
    def run(*args):
        ...
        for passthrough in ("FAKE_DEFAULT_TARGET", "OZZYTV_SITE"):
...
class TestTheNetworkShare:
    ...
    def test_guests_are_refused_rather_than_quietly_allowed(self):
        """Without `map to guest = never` a wrong password becomes an anonymous
        login instead of a refusal, which is the failure nobody notices."""
        body = (REPO / "install" / "share.sh").read_text()
        assert "map to guest = never" in body
        assert "guest ok = no" in body
        assert "security = user" in body
    ...
    def test_files_arrive_owned_by_the_user_the_television_runs_as(self):
        """Otherwise they land as somebody else's and the menu stays empty."""
        body = (REPO / "install" / "share.sh").read_text()
        assert "force user = $OWNER" in body
        assert "valid users = $OWNER" in body
    ...
    def test_the_installer_mentions_it(self, install):
        assert "share.sh" in install().stdout
