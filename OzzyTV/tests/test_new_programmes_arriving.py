...
defensible: the only way to add a programme was to walk over with a memory
...
def play_something(app) -> None:
    """Press OK until a programme is on. How many presses that takes depends on
...
def arrive(library_dir, rel: str) -> None:
    ...
    p.write_bytes(b"a new programme")
...
class TestItNoticesOnItsOwn:
    ...
    def test_it_does_not_scan_on_every_tick(self, tv, clock, monkeypatch):
        ...
        looks = []
        real = tv.look_for_new_programmes
        monkeypatch.setattr(tv, "look_for_new_programmes",
...
class TestItDoesNotSnatchTheScreenAway:
    ...
    def test_the_refresh_reports_whether_anything_changed(self, tv, library_dir):
        assert tv.look_for_new_programmes() is False
        arrive(library_dir, "Hey Duggee/Episode 1.mp4")
        assert tv.look_for_new_programmes() is True
        assert tv.look_for_new_programmes() is False
