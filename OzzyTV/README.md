...
A memory stick works. Over the network is easier:

```sh
sudo ./install/share.sh              # set it up
...
| One user | the account that owns the media, with a Samba password of its own — **not** the Pi's login password |
| No guests | `map to guest = never`, so a wrong password is refused rather than quietly downgraded to anonymous |
